		...
		if content.previous_sibling.span != None:
			...
			if time > 10:
				...
				if content.previous_sibling.span.string == 'HD':
					...
					print("begin to download %s quality: 720P video duration:%d" %(title,time))
				else:
					HD_m3u8_url = m3u8_url[0].replace('hls.','hls-1080p.')										#选取出HD的m3u8文件文件地址
					print("begin to download %s quality: 1080P" %title)
